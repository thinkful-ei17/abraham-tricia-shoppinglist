...
function generateItemHtml(index) {
  return `<li>
        <span class= 'shopping-item ${
          STORE[index].checked ? "shopping-item__checked" : ""
        }' data-item-index="${index}">${STORE[index].name}</span>
...
function handleItemCheckClicked() {
  ...
  $(".js-shopping-list").on("click", ".shopping-item-toggle", function(event) {
    ...
    if (STORE[itemIndex].checked) STORE[itemIndex].checked = false;
    else STORE[itemIndex].checked = true;
  });
  renderShoppingList();
...
function handleSearchItemClicked() {
  ...
  $("#js-shopping-list-search").on(
    "submit" /*,'button[type="search"]'*/,
    event => {
      ...
      if (itemIndex > 0) {
        let item = $(".js-shopping-list").find(
          `span[data-item-index="${itemIndex}"]`
        );
...
function handleHideCompletedClicked() {
  $("#hide-completed").on("click", function() {
    if ($("#hide-completed")[0].checked) {
      let filteredList = STORE.map(
        (item, index) => (item.checked === false ? generateItemHtml(index) : "")
      );
